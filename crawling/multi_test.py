...
def select_option():
    ...
    return table_name, max_pages, max_element, conn, cur
#  return table_name, max_pages, max_element, conn, cur

def refresh():
    print("Refreshed!")
    driver.refresh();
    timer = threading.Timer(600, refresh)
    timer.start()

# url 가져오기
def get_url(max_pages):
    for page in tqdm(range(1, 6), desc="Progress of the pages", mininterval=1):
        try:
            refresh()
            # 자바스크립트 구동하기
...
if __name__ == '__main__':
    ...
    table_name, max_pages, max_element, conn, cur = select_option()
    ...
    pool(get_url(max_pages))

    end_time = time.time()
    print("저장된 데이터 : " + str(max_element))
